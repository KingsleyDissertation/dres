...
network.import_from_netcdf("DATA/outputs/network_wave5MW_tidal7.2MW.nc")
...
# Wave power - available resource (p_max_pu * p_nom), not solved dispatch
wave_p = pd.DataFrame({
    g: network.generators_t.p_max_pu[g].clip(0, 1) * network.generators.loc[g, "p_nom"]
    for g in wave_gens
}).sum(axis=1)

# Tidal power - available resource (p_max_pu * p_nom), not solved dispatch
...
print("=== ORKNEY ENERGY BALANCE - 5MW WAVE + 7.2MW TIDAL (2019) — CORRECTED DEMAND ===")
...
axes[0].set_title("Orkney: Wind Generation vs Demand (2019) — Corrected Demand Profile")
axes[0].legend()
...
plt.tight_layout()
plt.savefig("DATA/outputs/network_wave5MW_tidal7.2MW_CORRECTED.png", dpi=150)
plt.show()
print("Plot saved to DATA/outputs/network_wave5MW_tidal7.2MW_CORRECTED.png")